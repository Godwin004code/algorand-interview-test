...
const SingleAlgo = ({assetID, available, name, logo, unitname1}) => {
  return (
    <div className='border-solid border-zinc-200 border-2 rounded-md w-[18%] mb-2 flex flex-col justify-center p-3 md:w-[30%] sm:w-[50%]'>
        <div>
            <img className='w-[30px] mx-auto h-[50px]' src={logo} alt={name} />
        </div>
        <h2 className='font-medium text-xl text-center my-2'>{name}</h2>
